...
class NumeroLetra
{
    ...
    function letra() {
        ...
        if (strlen($cnumero) > 15) {
            ...
            /* Se agregan los centavos a letras */
            /*
              $cents = substr($cnumero, -2);
              $hay_significativo = false;
              for ($pos = 0; $pos < 2; $pos++) {
              // Control existencia Dígito significativo
              if (!($hay_significativo) && (substr($cnumero, $pos, 1) == '0'))
              ...
              // Detectar Tipo de Dígito
              switch ($pos % 2) {
              case 0: $centavos.=$this->letraDecena($pos, $cents);
              break;
              case 1: $centavos.=$this->letraUnidad($pos, $cents);
              break;
              }
              }
              // Detectar caso 0
              if ($centavos == '') {
              $centavos = $this->aTexto[5][3];
              }

              $cadena = $this->prefijo . $texto . " " . $this->moneda . " CON " . $centavos . " " . $this->sufijo
             */
            $cadena = strtoupper($this->prefijo . $texto . " " . $this->moneda . " CON " . substr($cnumero, -2) . "/100" . $this->sufijo);

            if ($this->mayusculas) {//mayusculas
                //$texto = strtoupper($this->prefijo . $texto . " " . $this->moneda . " CON " . substr($cnumero, -2) . "/100 " . $this->sufijo);
                $texto = strtoupper($cadena);
            } else {//minusculas
                //$texto = strtolower($this->prefijo . $texto . " " . $this->moneda . "  con" . substr($cnumero, -2) . "/100 " . $this->sufijo);
